...
impl ReliabilityWrapper {
    ...
    pub fn wrap_payload(&mut self, payload: &[u8]) -> io::Result<(MessageId, Vec<u8>)> {
        // 10 is an overapproximation for our header size, but good enough for now
    ...
    fn send_bytes(&mut self, msg: &[u8]) -> io::Result<MessageId> {
    ...
    fn recv_with_timeout<F>(&mut self, timeout: Option<Duration>, handler: F)
        where F: FnMut(ReceiveEvent)
    {
        ...
        self.receive_packet(&data.0, handler);
    }
